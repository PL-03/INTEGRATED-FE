...
<template>
  <div class="Status-content bg-gradient-to-t from-gray-500 to-blue-100/20 min-h-screen">
    <nav class="h-20 flex justify-between shadow-md bg-blue-950 fixed left-0 right-0 top-0 z-10 min-w-screen">
      ...
      <div class="flex m-4">
        <button
          ...
          <svg class="mr-1" xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 20 20">
    ...
    <div class="mt-4 flex justify-center items-center">
      <table class="table-auto w-9/12 m-2 rounded-2xl overflow-hidden shadow-md shadow-white/40">
        ...
        <tbody>
          <tr v-for="(status, index) in statuses" :key="index"
            :class="index % 2 === 0 ? 'bg-yellow-50' : 'bg-orange-100'" class="itbkk-item font-mono border-b">
            <td class="px-4 py-2 whitespace-nowrap text-m font-medium text-gray-900 border-r">
              {{ index + 1 }}
            </td>
            <td class="text-m text-gray-900 font-light px-4 py-2 whitespace-normal border-r itbkk-status-name">
              {{ status.name }}
            </td>
            ...
            <td class="px-4 py-2">
              <button class="text-blue-800 hover:text-blue-500 mb-2 e-btn edit-delete itbkk-button-edit"
                @click="handleEditStatus(status)" :disabled="status.id === 1">
              ...
              <button class="text-red-700 hover:text-red-400 d-btn px-4 edit-delete itbkk-button-delete"
                @click="handleDeleteStatus(status)" :disabled="status.id === 1">
    ...
    <ConfirmationModal :show="showTransferModal" :task-title="statusToDelete?.name" :is-status="true"
      :statuses="statuses" :tasks-associated="true" @close="closeTransferModal" @transfer="transferTasks" />
  </div>
</template>
