...
<template>
    <div class="itbkk-status-filter relative flex ml-28">
      <div
        class="itbkk-filter-item flex w-72  h-11 px-4 bg-white rounded-md border-gray-300 rounded-l focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-400 overflow-x-scroll">
        <div class="itbkk-filter-item mt-2 mb-1 whitespace-nowrap text-gray-500 text-sm  ">
          <strong class="">Filter by status :</strong></div>
        ...
        <div v-for="(option, index) in selectedOptions" :key="index"
          ...
          <span>{{ option }}</span>
          <button type="button" class="itbkk-filter-item ml-1  text-gray-500 hover:text-red-700 focus:outline-none"
            @click="removeOption(index)">
            <svg class="itbkk-filter-item-clear h-3 w-3 fill-current" xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20">
              <path
      ...
      <div class=" relative">
        ...
        <div v-if="showDropdown" class="absolute left-0 z-10 mt-2 bg-white rounded-lg shadow-lg" @click="filter">
      ...
      <button v-if="selectedOptions.length > 0" type="button"
