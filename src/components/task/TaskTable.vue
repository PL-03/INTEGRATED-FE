...
const isToggled = ref(false); // to track the toggle status

const handleToggle = (event) => {
  if (event.target.checked) {
    showModal.value = true; // Show modal before changing to public
  } else {
    isToggled.value = false; // Set it back to private
  }
};
...
const confirmToggle = () => {
  isToggled.value = true; // Confirm changing to public
  showModal.value = false;
};
    ...
    <nav class="navbar h-20 flex justify-between z-10 shadow-md shadow-[#95999ad8]">
      ...
      <div class="flex m-4 items-center space-x-6">
        ...
        <button @click="handleStatusList"
          class="itbkk-manage-status flex items-centerd text-black hover:text-blue-600 transition duration-300">
          Manage Status
        </button>

        <div class="relative text-black">
          <button @click="toggleDropdown" class="itbkk-fullname flex items-center hover:text-[#4d5fcb]">
    ...
    <div class="flex justify-center">
      <div class="mt-14 w-fit">
        ...
        <div class="mb-14 text-3xl drop-shadow-lg">
          <span class="text-[#2b4483]">{{ boardName }}</span>
        </div>
        <div class="flex items-center justify-between my-2">
          <!-- <div class="flex"> -->
          <FilterDropdown @filter="handleFilterData" />
          <!-- </div> -->
          <label class="itbkk-board-visibility relative flex items-center cursor-pointer">
            <input type="checkbox" value="" class="itbkk-board-visibility sr-only peer" v-model="isToggled"
              @change="handleToggle" />
            <div class="itbkk-board-visibility w-11 h-6 bg-gray-400 border border-[#535459] peer-focus:outline-none
          peer-focus:ring-2 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-['']
          ...
          <table class="table-auto mb-2 rounded-md overflow-hidden">
            ...
            <tbody>
              <tr v-for="(task, index) in filteredTasks" :key="index"
                ...
                <td class="px-4">
                  <button class="text-blue-800 hover:text-blue-500 ml-2 e-btn itbkk-button-edit"
                  ...
                  <button class="text-red-700 hover:text-red-400 d-btn px-4" @click="handleDeleteTask(task)">
