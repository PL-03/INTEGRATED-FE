...
const confirmDeleteStatus = async () => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_BASE_URL}/v2/statuses/${statusToDelete.value.statusId
      }`,
      {
        method: "DELETE",
      }
  ...
  <div class="Status-content bg-gradient-to-t from-gray-500 to-blue-100/20 min-h-screen">
    ...
    <div class="mt-4 flex justify-center items-center">
      <table class="table-auto w-9/12 m-2 rounded-2xl overflow-hidden shadow-md shadow-white/40">
        ...
        <tbody>
          <tr v-for="(status, index) in statuses" :key="status.statusId" :class="index % 2 === 0 ? 'bg-yellow-50' : 'bg-orange-100'" class="itbkk-item font-mono border-b">
            ...
            <td class="text-m text-gray-900 font-light px-4 py-2 whitespace-normal border-r itbkk-status-description">
              {{ status.description || "No description provided" }}
            </td>
            <td class="px-4 py-2">
              <button class=" text-blue-800 hover:text-blue-500 mb-2 e-btn" @click="handleEditStatus(status)">
                <svg
              ...
              <button class=" text-red-700 hover:text-red-400 d-btn px-4" @click="handleDeleteStatus(status)">
                <svg
    ...
    <ConfirmationModal :show="showConfirmationModal" :statusName="statusToDelete?.name" @close="closeConfirmationModal"
